...
namespace DOL.GS.Scripts
{
    public class VotingMgr
    {
        ...
        private static readonly uint STD_VOTING_DURATION = 120;
    ...
    public class GMVoteCommandHandler : AbstractCommandHandler, ICommandHandler
    {
        public void OnCommand(GameClient client, string[] args)
        {
            ...
            switch (command)
            {
                ...
                case "list":
                    {
                        IList<DBVoting> votings;
                        if (param != string.Empty)
                            votings = DOLDB<DBVoting>.SelectObjects(DB.Column("VoteID").IsLike($"%{param}%"));
                        else
                            votings = DOLDB<DBVoting>.SelectAllObjects();
