...
namespace DOL.GS
{
    public class RealmpointMaster : GameNPC
    {
        ...
        public override bool Interact(GamePlayer player)
        {
            ...
            if (freeRR > 0)
            {
                player.Out.SendMessage("Greetings, " + player.Name + ", I can give you free [Realmrank " + freeRR + "]!", eChatType.CT_System, eChatLoc.CL_PopupWindow);
            }
            if (freeRR == 0)
            {
                player.Out.SendMessage("I'm sorry, " + player.Name + " I've been told not to grant anyone free realmranks!", eChatType.CT_Say, eChatLoc.CL_PopupWindow);
            }
